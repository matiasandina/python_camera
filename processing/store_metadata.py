...
def get_session(video_path, type = "str"):
    ...
    if match:
        ...
        if type == "dt":
            timestamp_dt
        # Step 2: Convert the extracted timestamp to a datetime object


        # Step 3: Format the datetime object to the desired format YYYY-MM-DD
            return timestamp_dt
    else:
...
def write_parquet(animal_dir, data):
    ...
    filename = f"{animal_id}_metadata.parquet"
